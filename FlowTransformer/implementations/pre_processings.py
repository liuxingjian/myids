...
class StandardPreProcessing(BasePreProcessing):
    ...
    def fit_categorical(self, column_name: str, values: np.array):
        ...
        # Limit the levels to specific sizes to match C++ producer sizes
        max_levels = self.n_categorical_levels
        if column_name == 'PROTOCOL':
            max_levels = 3 - 1
        elif column_name == 'L7_PROTO':
            max_levels = 19 - 1
        elif column_name == 'TCP_FLAGS':
            max_levels = 9 - 1
        elif column_name == 'L4_SRC_PORT' or column_name == 'L4_DST_PORT':
            max_levels = 32 - 1

        self.encoded_levels[column_name] = [s[0] for s in sorted_levels[:max_levels]]
    ...
    def transform_categorical(self, column_name:str, values: np.array, expected_categorical_format: CategoricalFormat):
        ...
        # Set specific dummy counts for specific features to match C++ producer sizes (1x8x3, 1x8x19, 1x8x9)
        num_classes = len(encoded_levels) + 1
        if column_name == 'PROTOCOL':
            num_classes = 3
        elif column_name == 'L7_PROTO':
            num_classes = 19
        ...
        v = to_categorical(result_values, num_classes=num_classes)
        return pd.DataFrame(v, columns=[f"{column_name}_{i}" for i in range(num_classes)])
