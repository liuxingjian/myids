...
class TransformerEncoderBlock(layers.Layer):
    def __init__(self, input_dimension:int, inner_dimension:int, num_heads:int, dropout_rate=0.1, use_conv:bool=False, prefix:str=None, attn_implementation:MultiHeadAttentionImplementation = MultiHeadAttentionImplementation.GPT3):
        ...
        # The second FFN projection should be linear to keep residual statistics stable.
        self.feed_forward_1 = Conv1D(filters=inner_dimension, kernel_size=1, activation="linear", name=f"{prefix}feed_forward_1") \
            if use_conv else Dense(inner_dimension, activation="linear", name=f"{prefix}feed_forward_1")
