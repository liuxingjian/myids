...
class StandardPreProcessing(BasePreProcessing):
    ...
    def fit_numerical(self, column_name: str, values: np.array):
        ...
        self.min_range[column_name] = (v0, r)  # v0 : min value of `values`, r : range between min and max
    ...
    def fit_categorical(self, column_name: str, values: np.array):
        levels, level_counts = np.unique(values, return_counts=True)   # level_counts : the number of times each level appears in `values`
        sorted_levels = list(sorted(zip(levels, level_counts), key=lambda x: x[1], reverse=True)) # sort the levels by the number of times they appear in `values`
        self.encoded_levels[column_name] = [s[0] for s in sorted_levels[:self.n_categorical_levels]] # take the top `n_categorical_levels` levels, also means the top n most frequent levels


    def transform_categorical(self, column_name:str, values: np.array, expected_categorical_format: CategoricalFormat):
        encoded_levels = self.encoded_levels[column_name]  # the top `n_categorical_levels` levels
        ...
        for level_i, level in enumerate(encoded_levels):
            level_mask = values == level  # mask of the values that are equal to the current level
        ...
        v = pd.get_dummies(result_values, prefix=column_name) # one-hot encoding
        return v
